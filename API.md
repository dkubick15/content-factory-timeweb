...
Публикация в Telegram (текст и/или медиа) через защищённый серверный
ретранслятор. Bot Token не передаётся в браузер. Ограничен publish rate limit.

## Серверное расписание
Встроенный планировщик проверяет очередь каждую минуту. Наступившая публикация
подписывается HMAC и отправляется через ретранслятор; при временной ошибке
