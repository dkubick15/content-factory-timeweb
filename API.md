...
## POST /api/scheduler/telegram/claim
Защищённый GitHub OIDC endpoint. Выдаёт наступившие публикации Telegram
официальному workflow из `main`, блокируя их от повторной отправки.

## POST /api/scheduler/telegram/complete
Защищённый GitHub OIDC endpoint. Сохраняет успешный `message_id` или ставит
неудачную публикацию на повторную попытку.
